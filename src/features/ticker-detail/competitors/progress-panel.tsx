...
import { useCompetitorsProgress } from './use-competitors-progress'

export function ProgressPanel({ ticker, active }: { ticker: string; active: boolean }) {
  const event = useCompetitorsProgress(ticker, active)
  ...
  return (
    <div className="space-y-1.5">
      <Progress value={pct} className={cn('h-1.5', isFailed && '[&>div]:bg-red-500')} />
      <p
        className={cn(
